const APP = {
  ...
  renderNav() {
    ...
    const me = this.getMe();
    let links = session
      ? `<a href="discover.html">discover</a><a href="profile.html">profile</a><a href="settings.html">settings</a>`
      : `<a href="login.html">log in</a>`;
    nav.innerHTML = `
      <a href="index.html" class="nav-logo">musical</a>
      <div class="nav-links">
        ${links}
        ${session ? `<a href="profile.html" class="nav-user"><div class="nav-avatar">${me ? me.name.charAt(0).toUpperCase() : '?'}</div></a>` : ''}
      </div>
    `;
    ...
    accounts.forEach(user => {
      ...
      row.innerHTML = `
        ...
        <div class="user-info">
          ...
          <p>@${user.username} · ${user.genres.slice(0,2).join(', ')}</p>
        </div>
        <div class="user-meta">
  ...
  renderProfilePage() {
    ...
    document.getElementById('p-tracks').innerHTML = (user.recentTracks || []).map(t => `
