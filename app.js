const APP = {
  ...
  platforms: {
    ...
    github:    { icon: 'fab fa-github',      color: '#fff',    label: 'GitHub' },
    ...
    tiktok:    { icon: 'fab fa-tiktok',      color: '#fff',    label: 'TikTok' },
    ...
    website:   { icon: 'fas fa-globe',       color: '#fff',    label: 'Website' },
  ...
  fonts: ['Inter', 'JetBrains Mono', 'Space Grotesk', 'Playfair Display', 'Bebas Neue', 'Creepster'],
  cursors: ['default', 'crosshair', 'pointer', 'help', 'text', 'wait', 'move', 'not-allowed'],
  ...
  getAccounts() {
    ...
    // Migrate old accounts to new schema
    accounts.forEach(u => {
  ...
  renderNav() {
    ...
    nav.innerHTML = `
      ...
      <div class="nav-links">
        ${links}
        ${session ? `<a href="profile.html" class="nav-user"><div class="nav-avatar" style="${me?.avatar ? `background-image:url('${me.avatar}');background-size:cover;background-position:center;` : ''}">${me?.avatar ? '' : (me ? me.name.charAt(0).toUpperCase() : '?')}</div></a>` : ''}
      </div>
    `;
    ...
    accounts.forEach(user => {
      ...
      row.innerHTML = `
        ...
        <div class="user-info">
          ...
          <p>@${user.username} · ${user.genres.slice(0,2).join(', ')} · ${user.views} views</p>
        </div>
        <div class="user-meta">
  ...
  renderProfilePage() {
    ...
    // Apply user's theme/font/cursor
    this.applyUserTheme(user);

    document.getElementById('p-avatar').innerHTML = user.avatar 
      ? `<img src="${user.avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none';this.parentElement.textContent='${user.name.charAt(0).toUpperCase()}'">`
      : user.name.charAt(0).toUpperCase();
    document.getElementById('p-name').innerHTML = `${user.name}${this.renderBadgesInline(user.badges)}`;
    ...
    if (bannerEl) {
      if (user.banner) {
        bannerEl.innerHTML = `<img src="${user.banner}" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none';this.parentElement.style.background='${this.themes[user.theme]?.gradient || this.themes.blue.gradient}'">`;
      } else {
    ...
    if (actions) {
      if (isSelf) {
        ...
        else actions.innerHTML = `<button class="btn btn-primary btn-sm" onclick="APP.sendRequest('${user.id}');location.reload()">add friend</button>`;
      } else {
  ...
  renderSettings() {
    ...
    if (fontContainer) {
      fontContainer.innerHTML = this.fonts.map(f => `
        <button class="font-btn ${me.font === f ? 'active' : ''}" onclick="APP.setFont('${f}')" style="font-family:'${f}',sans-serif;">${f}</button>
      `).join('');
    }
    ...
    if (cursorContainer) {
      cursorContainer.innerHTML = this.cursors.map(c => `
        <button class="cursor-btn ${me.cursor === c ? 'active' : ''}" onclick="APP.setCursor('${c}')">${c}</button>
      `).join('');
    }
  ...
  setTheme(theme) {
    this.updateMe({ theme, accent: this.themes[theme].accent });
  ...
  applyUserTheme(user) {
    ...
    document.documentElement.style.setProperty('--accent', user.accent || theme.accent);
    document.documentElement.style.setProperty('--accent-hover', user.accent || theme.accent);
    document.documentElement.style.setProperty('--bg', theme.bg);
    if (user.font) document.body.style.fontFamily = `${user.font}, sans-serif`;
    if (user.cursor) document.body.style.cursor = user.cursor === 'default' ? 'auto' : user.cursor;
  },
